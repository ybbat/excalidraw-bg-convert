...
const hue_rotate = (col: Colord, angle: number): Colord => {
    ...
    let rgba_mat = matrix([[col["parsed"]["r"]], [col["parsed"]["g"]], [col["parsed"]["b"]], [col["parsed"]["a"]], [1]]);
    const rad = angle * pi / 180;

    // Constant matrices
    const a = matrix([[0.213, 0.715, 0.072], [0.213, 0.715, 0.072], [0.213, 0.715, 0.072]])
    const b = matrix([[0.787, -0.715, -0.072], [-0.213, 0.285, -0.072], [-0.213, -0.715, 0.928]])
    const c = matrix([[-0.213, -0.715, 0.928], [0.143, 0.140, -0.283], [-0.787, 0.715, 0.072]])
    ...
    const hue_rotate = subtract(add(identity(5), terms.resize([5, 5])), (identity(3) as Matrix).resize([5, 5]));
